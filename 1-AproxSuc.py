...
Proyecto final.
Programa 1: AproxSuc.py
Calcula la raiz de una ecuacion, utilizando el metodo de Aproximaciones sucesivas.
'''
...
# Inicio del programa.
print("Bienvenido a AproxSuc. Este programa calcula la raiz de una ecuacion, utilizando el metodo de Aproximaciones sucesivas.")

# Se obtienen la funcion y el despeje a utilizar. Se verifica que no esten mal escritos.
try:
  ecuacion = input("\nEscribe la ecuacion de la funcion: ")
  ecuacion = sympify(ecuacion, evaluate = False)
  despeje = input("\nIntroduce un despeje de la misma ecuacion: ")
  despeje = sympify(despeje, evaluate = False)
...
# Si la ecuacion esta mal escrita, indica el error y termina el programa. Si esta bien, continua con el programa.
  ...
  if(x0):
    ...
    convergio = False
    iteraciones = 0
    diferencia = abs(N(despeje.subs(x, x0)) - x0) 
    ...
    try:
      while convergio == False:
        ...
        x1 = N(despeje.subs(x, x0))
        iteraciones = iteraciones + 1
        
        # Revisa si el metodo esta convergiendo al calcular la diferencia entre las dos aproximaciones mas recientes. 
        # Si la diferencia esta aumentando, quiere decir que el metodo esta divergiendo.
        nueva_diferencia = abs(x1 - x0)
        if nueva_diferencia > diferencia:
          raise ValueError

        # Si el metodo esta convergiendo continua con el siguiente codigo.
        else:
          ...
          if abs(diferencia / x1) < (1 * 10 ** (0 - decimales)):
            convergio = True
    ...
    # Si el metodo esta divergiendo termina el programa.
    except ValueError:
      print("\nError: El metodo no esta convergiendo.")

    # Cuando se alcanzo un resultado convergente, transforma el valor de 'x1' a una cadena solo con los decimales deseados.
    else:
  ...
  # Si no se encontro un valor para 'x0', termina el programa.
    ...
    print("\nError: No hubo cambio de signo en el intervalo.")
...
despeje: sympify, despeje de la ecuacion del problema, utlizado para aproximarse a la raiz como indica el metodo de Aproximaciones sucesivas.
...
diferencia: float, representa el valor absoluto de la diferencia entre el valor de 'x0' y el valor de 'x1' de la iteracion pasada, y es usada para revisar si el metodo esta convergiendo o no.
...
nueva_diferencia: float, representa el valor absoluto de la diferencia entre el valor de 'x0' y el valor de 'x1' en la iteracion actual, y es usada para revisar si el metodo esta convergiendo o no.
