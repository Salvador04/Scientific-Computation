...
Proyecto final.
Programa 10: RungeK.py
Calcula una solucion aproximada de una ecuacion diferencial, empleando el metodo de Runge-Kutta.
'''
...
# Inicio del programa.
print("Bienvenido a RungeK. Este programa calcula una solucion aproximada de una ecuacion diferencial, empleando el metodo de Runge-Kutta.")
...
# Si la ecuacion esta mal escrita, indica el error y termina el programa. Si esta bien, continua con el programa.
	...
	if (valor_x - valor_x0) == 0:
		...
		# Realiza las iteraciones correspondientes al metodo de Runge-Kutta, mejorando la aproximacion por cada iteracion.
		for i in valores_x:

			# Introduce los valores actuales de las variables 'x' y 'y' en la tabla de resultados.
			resultados.append([i, valor_y0])

			# Calcula los valores de los terminos 'k1', 'k2', 'k3' y 'k4' para calcular el nuevo valor de 'y'.
			k1 = incremento * N(ecuacion.subs([(x, i), (y, valor_y0)]))
			k2 = incremento * N(ecuacion.subs([(x, i + incremento / 2), (y, valor_y0 + k1 / 2)]))
			k3 = incremento * N(ecuacion.subs([(x, i + incremento / 2), (y, valor_y0 + k2 / 2)]))
			k4 = incremento * N(ecuacion.subs([(x, i + incremento), (y, valor_y0 + k3)]))
			valor_y0 = valor_y0 + (1 / 6) * (k1 + 2 * k2 + 2 * k3 + k4)
...
k1: float, variable que almacena el valor del primer termino para calcular el valor en 'y' para cierta 'x', como indica el metodo de Runge-Kutta.

k2: float, variable que almacena el valor del segundo termino para calcular el valor en 'y' para cierta 'x', como indica el metodo de Runge-Kutta.

k3: float, variable que almacena el valor del tercer termino para calcular el valor en 'y' para cierta 'x', como indica el metodo de Runge-Kutta.

k4: float, variable que almacena el valor del ultimo termino para calcular el valor en 'y' para cierta 'x', como indica el metodo de Runge-Kutta. 
...
valor_x: float, almacena el valor final de 'x', requerido por el metodo de Runge-Kutta.

valor_x0: float, almacena el valor inicial de 'x', requerido por el metodo de Runge-Kutta.
